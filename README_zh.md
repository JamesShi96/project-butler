...
| "做完了" / "收工" / "end session" | 自动写会话日志、更新接手指引、同步 Wiki、整理文件。说完走人，一切都已记录。 |
| "继续" / "接着上次" / "上次做到哪了" / "continue" | 恢复上次会话的完整对话。新开一个 session，直接从上次中断的地方继续——不用重新解释。 |
| "全面回顾" / "项目全景" / "continue full context" | 完整项目轨迹恢复。上次会话详细读取 + 所有历史会话摘要时间线。长时间没碰项目后回来用这个。 |
| "审查规则" / "review claude" | AI 展示工作中发现的候选规则，你逐条确认、驳回或改写。宪法从真实使用中有机生长。 |
| "同步一下" / "sync wiki" | 强制更新项目概览文件。任何人（或 AI）读这一个文件就能理解项目全貌。 |
| "项目现状" / "status" | 朗读 Wiki + handoff 摘要。 |
| "整理文件" / "organize files" | 智能文件整理——理解每个文件是什么、该放哪里，遵守你的命名规范。 |
...
运行一次 `/project-butler`，它会创建 9 个文件，按 3 个层级组织：
...
在 `/project-butler` 设置时选择，或随时说"切换语言"更改。
...
#### 6. 更新日志（UPDATE_LOG.md）

不是每个会话都值得记录到更新日志，但重要的那些不应该被遗漏。收工时，AI 自动评估本次会话是否包含重大更新（新功能、重大修改、3+ 文件变更、里程碑、重要 TODO 完成），满足条件则自动写入里程碑级条目：

```markdown
## 2026-05-05 — 用户认证模块完成

- 新增基于 JWT 的认证，含 refresh token
- 创建登录/注册/登出接口
- 编写集成测试（12/12 通过）
---
```

如果项目有 GitHub 远程仓库，还会询问是否创建 GitHub Release。代码项目和非代码项目都适用——任何有重大更新值得追踪的项目。
...
- 收工时自动判断是否为重大更新（新功能、重大修改、3+ 文件变更、里程碑等）
- 新增 `UPDATE_LOG.md` — 介于会话日志和 Wiki 之间的里程碑级变更记录
- GitHub Release 集成（可选，先询问用户）
- 初始化时在 README 添加链接（非内联）
- 代码项目和非代码项目都适用
