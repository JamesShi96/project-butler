...
启用 Project Profile System 后，它还会维护一个小型机器可读 profile，让 AI 知道哪些长期文档重要、哪些 section 受保护、哪些 profile 更新被暂时延后。
...
│  DOCS.md                            │  <- 文档索引
│  .claude/project-profile.json       │  <- 可选 Profile 配置
│  .claude/profile-pending.json       │  <- 可选 Profile debt 队列
└─────────────────────────────────────┘
            ↑ 从事实中总结
原始事实
┌──────────────────────┐ ┌───────────────────────┐
...
- **Project Profile** 可选记录项目形态、文档层级、文档策略、pending profile 更新和 review queue。
...
普通升级不会强制启用 Profile System。已有 profile 文件会被保留；当用户明确要求 profile setup、full close 或 foundation repair 时，project-butler 可以基于现有项目文档反推 profile，并在写入前请求确认。
...
- 让生成规则、continue、full context 恢复和 upgrade mode 识别并保留可选 profile 文件。
- 明确 Project Profile System 是可选 profile-aware 行为，不属于基础 7 组件记忆栈。
