...
如果项目需要更强的产品、架构、Roadmap、调研或 Eval 对齐，project-butler 也可以在初始化时创建 Project Profile，并在收工时支持 Normal Close / Full Close。
...
| `normal close` | 保存 session，并把影响 Profile 的更新放入 pending 队列。 |
| `full close` | 用有边界的 Scope Plan 立即对齐受影响的 Profile 文档。 |
| `profile setup` / `foundation repair` | 经确认后创建或修复项目 Profile 和基础参考文档。 |
...
└── .claude/
    ...
    ├── profile-pending.json    <- Profile pending / debt 队列
...
Project Butler 还会维护一个小型机器可读 profile，让 AI 知道哪些长期文档重要、哪些 section 受保护、哪些 profile 更新被暂时延后。
...
| Cursor | 项目规则，best-effort | project-butler 可以生成 `.cursor/rules/project-system.mdc`，让 Cursor 读取同一套项目记忆文件并镜像主要触发词。 |
| Codex | `AGENTS.md`，best-effort | project-butler 可以生成 `AGENTS.md`，让 Codex 读取同一套项目记忆文件并镜像主要触发词。 |
...
│  DOCS.md                            │  <- 文档索引
│  .claude/project-profile.json       │  <- Profile 配置
│  .claude/profile-pending.json       │  <- Profile debt 队列
└─────────────────────────────────────┘
            ↑ 从事实中总结
原始事实
┌──────────────────────┐ ┌───────────────────────┐
...
- **Project Profile** 记录项目形态、文档层级、文档策略、pending profile 更新和 review queue。

### Project Profiles

Project Profile System 是 setup 和 close 的内部 profile-aware 行为。Fresh setup 仍然是 conversational 的，也可以通过只创建少量已确认文档来保持轻量。

初始化时，project-butler 会让用户自然描述项目，AI 推断项目形态，问少量有价值的追问，然后提出 Required / Recommended / Optional 参考文档结构。它不会强迫用户选择固定项目类型，也不会把 Profile System 暴露成 setup 开关。

收工时，项目可以使用：

| 模式 | 行为 |
|---|---|
| Normal Close | 保存 session，并把影响 profile 的变化记录到 `.claude/profile-pending.json`。 |
| Full Close | 只读取受影响的 profile 文档，提出 Scope Plan，并在边界内做安全更新。 |

Full Close 确认的是边界，不是每一个小改动。修改 protected sections、document policies、stable baselines 或整段/整文档 rewrite 仍然需要明确确认。
...
升级时，project-butler 会保留已有文件，并建议把项目升级到当前 profile-aware setup 模型。它会基于现有项目文档反推 profile state，并在写入 profile 文件或修改已有文档策略前请求确认。
...
- 可选：Codex 或其他能读取项目 Markdown 文件和 `AGENTS.md` 的 AI 编程助手

## 更新日志

### v1.7.1 (2026-06-21) - 多工具兼容基础
- 将版本新鲜度检测抽到 `scripts/check-update.sh`，Claude Code 保持自动 Step -1 检查，Cursor/Codex 可按需手动检查。
- 新增 Codex `AGENTS.md` 项目指令模板，并让 Cursor 规则覆盖手动 update-check。
- 文档中明确 Cursor/Codex 是 best-effort 支持，并新增 adapter 覆盖矩阵。

### v1.6.0 (2026-06-10) - Project Profile System 运行接入
- 新增 `references/project-profile-system.md`，并把 profile setup、Normal Close、Full Close、Foundation Repair 和 profile-aware status 接入主 skill。
- 让生成规则、continue、full context 恢复和 upgrade mode 识别并保留 profile 文件。
- 明确 Project Profile System 是基础 7 组件记忆栈之上的内部 profile-aware runtime 行为。
